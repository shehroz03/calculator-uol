...
import { Plus, X, RotateCcw, Download, Moon, Sun, PlusCircle, GraduationCap, MessageCircle, Send, Share2, ShieldQuestion } from 'lucide-react';
...
function App() {
  ...
  // Use strictly firebase auth ID, do not default to guest
  const userId = user ? user.id : null;
  const [apiUrl, setApiUrl] = useState(() => localStorage.getItem('uol-api-url') || 'http://localhost:5000');
  ...
  useEffect(() => {
    ...
    // 2. Silently attempt to fetch cloud backend updates
    setIsFetchingData(true);
    fetch(`${apiUrl}/api/results/${userId}`, {
      headers: { 'x-api-key': 'uol-pro-secure-key-2026-secret' }
    })
      .then(res => res.json())
      .then(data => {
        if (data.semesters && data.semesters.length > 0) {
          ...
          setTotalCreditsDone(data.totalCreditsDone || '');
        }
      }).catch(err => console.log('Backend not connected, using offline save.'))
      .finally(() => setIsFetchingData(false));
  }, [userId, apiUrl]);

  useEffect(() => {
    if (!userId || isFetchingData || loadedUserId.current !== userId || isSandbox) return;
    ...
    localStorage.setItem('uol-api-url', apiUrl); // Save API URL
      ...
      try {
        const res = await fetch(`${apiUrl}/api/results`, {
          ...
          headers: {
            ...
            'x-api-key': 'uol-pro-secure-key-2026-secret'
          },
          body: JSON.stringify({ userId, currentGPA, totalCreditsDone, semesters })
        });
        if (res.ok) {
  ...
  }, [semesters, currentGPA, totalCreditsDone, userId, apiUrl]);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (data.reply) {
        ...
        setMessages([...newMsgs, { role: 'ai', content: "Sorry, something went wrong." }]);
      }
    } catch (err) {
      setMessages([...newMsgs, { role: 'ai', content: "Error: Could not connect to AI server. Make sure the backend is running." }]);
    } finally {
  ...
  return (
    <div className="container">
          ...
          <div className="main-card">
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex gap-2" style={{ alignItems: 'center' }}>
                <button onClick={() => setIsSandbox(!isSandbox)} style={{ border: 'none', background: 'none', cursor: 'pointer', color: isSandbox ? '#f59e0b' : 'var(--text-muted)' }} title="Sandbox Mode (Don't Save)">
                  <ShieldQuestion size={20} />
                </button>
                <button onClick={generateShareLink} style={{ border: 'none', background: 'none', cursor: 'pointer', color: 'var(--primary)' }} title="Share View-Only Link">
                  <Share2 size={20} />
                </button>
                {user ? (
                  <button onClick={() => {
            ...
            <div className="mb-8">
              ...
              <div style={{ fontSize: '10px', color: 'var(--text-muted)', marginTop: '8px', display: 'flex', justifyContent: 'space-between' }}>
                <span>{isFetchingData ? '⏳ Loading cloud data...' : (isSyncing ? '🔄 Syncing...' : (lastSynced ? `✅ Synced ${lastSynced}` : '☁️ Offline Mode'))}</span>
            ...
            <footer style={{ marginTop: '40px', textAlign: 'center', paddingBottom: '20px' }}>
              <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', fontSize: '0.7rem', fontWeight: 'bold', color: 'var(--primary)', textTransform: 'uppercase', cursor: 'pointer', marginBottom: '10px' }}>
                ...
                <span onClick={() => setShowConfig(true)}>Settings</span>
              </div>
      ...
      <AnimatePresence>
        ...
        {showConfig && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="modal-overlay" onClick={() => setShowConfig(false)}>
            <motion.div initial={{ y: 50 }} animate={{ y: 0 }} className="modal-content" onClick={e => e.stopPropagation()}>
              <button className="modal-close" onClick={() => setShowConfig(false)}><X size={20} /></button>
              <h3 className="section-title" style={{ margin: 0 }}>App Settings & Profile</h3>
              <p style={{ marginTop: '5px', fontSize: '0.8rem', color: 'var(--text-muted)' }}>Configure connection and your official transcript details.</p>

              <div style={{ marginBottom: '15px', marginTop: '15px' }}>
                <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '0.9rem' }}>Full Name</label>
                <input type="text" value={profile.name} onChange={(e) => setProfile({...profile, name: e.target.value})} placeholder="Your Name" style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid var(--border)', background: 'var(--bg-color)', color: 'var(--text-main)' }} />
              </div>
              <div style={{ marginBottom: '15px' }}>
                <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '0.9rem' }}>Registration Number</label>
                <input type="text" value={profile.rollNo} onChange={(e) => setProfile({...profile, rollNo: e.target.value})} placeholder="e.g. 12345678" style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid var(--border)', background: 'var(--bg-color)', color: 'var(--text-main)' }} />
              </div>
              <div style={{ marginBottom: '15px' }}>
                <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '0.9rem' }}>Degree Program</label>
                <input type="text" value={profile.program} onChange={(e) => setProfile({...profile, program: e.target.value})} placeholder="e.g. BS Computer Science" style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid var(--border)', background: 'var(--bg-color)', color: 'var(--text-main)' }} />
              </div>

              <div style={{ marginBottom: '15px' }}>
                <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '0.9rem' }}>Server URL</label>
                <input
                  type="text"
                  value={apiUrl}
                  onChange={(e) => setApiUrl(e.target.value)}
                  placeholder="http://192.168.1.5:5000"
                  style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid var(--border)', background: 'var(--bg-color)', color: 'var(--text-main)' }}
                />
                <p style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: '5px' }}>
                  If using on Mobile, enter your PC's IP address (e.g. http://192.168.1.5:5000).
                  Ensure both devices are on the same Wi-Fi.
                </p>
              </div>

              <button className="btn-action" onClick={() => { localStorage.setItem('uol-api-url', apiUrl); setShowConfig(false); window.location.reload(); }}>Save & Reload</button>
            </motion.div>
          </motion.div>
        )}
        {showPrivacy && (
